...
const SingleRecipe = () => {
  ...
  const unfavHandler = () => {
    const filterData = favourite.filter((item) => item.id !== recipe.id);
    setFavourite(filterData);
    localStorage.setItem("favourite", JSON.stringify(filterData));
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-100 to-gray-200 flex items-center justify-center p-6">
      <div className="w-full max-w-6xl bg-white/70 backdrop-blur-lg shadow-2xl rounded-3xl overflow-hidden grid md:grid-cols-2">
        ...
        <div className="relative h-96 md:h-auto">
          ...
          <div>
            {favourite.find((item) => item.id === recipe.id) ? (
              <i
                onClick={unfavHandler}
                className="ri-heart-fill absolute top-4 right-4 text-red-400 text-3xl cursor-pointer"
              ></i>
            ) : (
              <i
                onClick={favHandler}
                className="ri-heart-line absolute top-4 right-4 text-red-400 text-3xl cursor-pointer"
              ></i>
            )}
          </div>
        </div>
